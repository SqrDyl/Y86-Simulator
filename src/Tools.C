...
uint64_t Tools::getByte(uint64_t source, int32_t byteNum)
{
  ...
  uint64_t masku = source >> (byteNum * 8);
   uint64_t maskl = 0x00000000000000FF;
  return masku & maskl;
}
...
uint64_t Tools::flipBits(uint64_t source, int32_t low, int32_t high)
{
  if (high < low || low < 0 || high > 63)
  {
   ...
   uint64_t hold = 1;
   uint64_t bits = ((hold << (high - low + 1)) - 1);
   bits = bits << low;
                         ...
                         int32_t srclow, int32_t dstlow, int32_t length)
{
  ...
  uint64_t srBits = getBits(source, srclow, srclow + length - 1);
  uint64_t one = 1;
  
  dest = dest & ~(((one << length) - 1) << dstlow);

  dest = dest | (srBits << dstlow);
...
uint64_t Tools::setByte(uint64_t source, int32_t byteNum)
{
  ...
  uint64_t hex = 0xFF;
  uint64_t bits = hex << (byteNum * 8);

  return (source & (~bits)) | (hex << byteNum * 8);
}
...
bool Tools::addOverflow(uint64_t op1, uint64_t op2)
{
  ...
  uint64_t signA = sign(op1 + op2);
  uint64_t sOp1 = sign(op1);
  uint64_t sOp2 = sign(op2);
...
uint64_t Tools::maxPos()
{
  ...
  result = result << 4;
  result += masksub; 
 // is 7FFFFFFFFFFFFF == only 15 need 16
  result = result << 4;
  result += 0xF;
